...
export const WeatherCard: React.FC = () => {
  ...
  const handleSaveLocation = (event: React.FormEvent) => {
    ...
    if (!city) return;
    setLocationStatus('ready');
    setSavedLocation({ city, region: '', country: '', latitude: 0, longitude: 0, source: 'manual' });
    ...
    <div className="desk-surface p-5 rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white/90 dark:bg-[#151c2e] shadow-soft-sm space-y-4">
      <div className="flex items-center justify-between border-b border-slate-200/50 dark:border-slate-800/50 pb-3">
        <div className="flex items-center space-x-2.5">
          ...
          <div><h3 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">Around You</h3><p className="text-[11px] text-slate-500 dark:text-slate-400">Local Weather</p></div>
        </div>
      ...
      {locationStatus === 'requesting' && !savedLocation ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between"><div><div className="flex items-center space-x-1.5"><MapPin className="w-3.5 h-3.5 text-amber-500" /><span className="text-sm font-bold text-slate-900 dark:text-white">{displayLocation?.city}{displayLocation?.country ? `, ${displayLocation.country}` : ''}</span></div><p className="text-[11px] text-slate-500 dark:text-slate-400 capitalize mt-0.5">{data.weather.description}</p></div><div className="flex items-center space-x-2"><img src={`https://openweathermap.org/img/wn/${data.weather.icon}@2x.png`} alt={data.weather.condition} className="w-10 h-10" /><span className="text-2xl font-extrabold text-slate-900 dark:text-white">{data.weather.temp}°C</span></div></div>
      ...
      {isModalOpen && <div className="fixed inset-0 z-50 bg-slate-950/40 backdrop-blur-xs flex items-center justify-center p-4"><div className="desk-surface p-6 rounded-3xl max-w-sm w-full bg-white dark:bg-[#151c2e] border border-slate-200 dark:border-slate-800 shadow-xl space-y-4"><div className="flex items-center justify-between"><h4 className="text-sm font-bold text-slate-900 dark:text-white">Set Your Location</h4><button onClick={() => setIsModalOpen(false)}><X className="w-4 h-4 text-slate-400" /></button></div><form onSubmit={handleSaveLocation} className="space-y-3"><div className="relative"><Search className="w-4 h-4 text-slate-400 absolute left-3 top-3" /><input type="text" required placeholder="Search for a city" value={cityInput} onChange={(event) => setCityInput(event.target.value)} className="w-full glass-input rounded-xl pl-9 pr-3 py-2 text-xs" /></div><div className="flex items-center justify-end gap-2"><button type="button" onClick={() => setIsModalOpen(false)} className="px-3 py-1.5 rounded-xl text-xs font-semibold text-slate-600 dark:text-slate-300">Cancel</button><button type="submit" className="px-4 py-1.5 rounded-xl bg-indigo-600 text-white text-xs font-semibold">Search City</button></div></form></div></div>}
    </div>
  );
};
